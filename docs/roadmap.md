...
| 2 | Core types & mixins | Arrays, hashes, symbols, ranges; `Comparable`/`Enumerable` and module inclusion. **Symbols, Array, ordered Hash, Range, `Comparable`/`Enumerable` (written in an embedded-Ruby prelude on top of `<=>`/`each`), `&&`/`\|\|`, `do…end` blocks, a broad String method set, and Integer/Float numeric methods (with Comparable mixed into Integer/Float/String, and Enumerable into Array/Range/Hash, with block auto-splat and `{name: value}` label hash literals, plus `break`/`next` for blocks and loops, compound assignment, top-level instance variables, and a broad Array method set) done; `method_missing` now gets a Symbol, matching MRI.** | **In progress** |
| 3 | Control flow, exceptions & Fiber | Real exception objects and status-return propagation; blocks/procs; Fiber-on-goroutine. **Exception hierarchy, `is_a?`/`kind_of?`/`instance_of?`, and `raise` done; `begin`/`rescue`/`ensure` next.** | **In progress** |
