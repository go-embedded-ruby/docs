...
| 3 | Control flow, exceptions & Fiber | Real exception objects and status-return propagation; Fiber-on-goroutine. **Exception hierarchy, `is_a?`/`kind_of?`/`instance_of?`, `raise`, `begin`/`rescue`/`ensure`/`else`/`retry` (block- and method-level), and non-local `break`/`next` done.** (Remaining: Fiber, Enumerator, `loop`, `lazy`.) | **In progress** |
