...
| 2 | Core types & mixins | Arrays, hashes, symbols, ranges; `Comparable`/`Enumerable` and module inclusion. **Symbols, Array, ordered Hash, Range, and `Comparable`/`Enumerable` (written in an embedded-Ruby prelude on top of `<=>`/`each`) done; `method_missing` now gets a Symbol, matching MRI.** | **In progress** |
