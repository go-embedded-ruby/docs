...
| 2 | Core types & mixins | Arrays, hashes, symbols, ranges; `Comparable`/`Enumerable` and module inclusion. **Symbols (`:name`) and Array (literals, indexing, each/map/push/include?, ==) done; `method_missing` now gets a Symbol, matching MRI.** | **In progress** |
